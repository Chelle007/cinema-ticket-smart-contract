...
export default Canister({
  addMovie: update(
    [text, nat32, int32, text, int32, int32],
    Result(Movie, CinemaTicketError),
    (name, price, durationMinutes, firstShowTime, showAmount, seatAmount) => {
      // CHECK ERROR
      if (durationMinutes <= 0 || showAmount <= 0 || seatAmount <= 0) {
        return Err({
          NotPositiveInt:
            'durationMinutes, showAmount, seatAmount must be positive integer.',
        });
      }
      if (!isValidTimeFormat(firstShowTime)) {
        return Err({
          TimeFormatError:
            "Please use 'HH:mm' format to input firstShowTime.",
        });
      }
      ...
      if (durationMinutes > 720) {
        return Err({
          DurationError:
            'The movie duration should be less than 12 hours(720 minutes).',
        });
      }

      const maxAmount =
        (timeToMinutes('24:00') - timeToMinutes(firstShowTime)) /
        (durationMinutes + 30); // add 30 minutes as break for cleaning the studio
      ...
      for (let i = 0; i < showAmount; i++) {
        ...
        const startTime = minutesToTime(
          timeToMinutes(movie.firstShowTime) + i * (durationMinutes + 30)
        );
  ...
  bookTicket: update(
    [Principal, int32],
    Result(Schedule, CinemaTicketError),
    (scheduleId, seats) => {
      // CHECK ERROR
      ...
      if (seats <= 0) {
        return Err({
          NotPositiveInt: 'seats must be a positive integer.',
        });
      }
